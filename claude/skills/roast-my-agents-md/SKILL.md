...
### Step 7: Run the A/B Tests (via the Workflow)

The A/B trial — clean-baseline protocol, the **hard barrier** between the
baseline and with-instruction runs, and the JS-diff that renders a verdict per
rule — is encoded as a **deterministic Workflow** so the fan-out, the
move-aside/restore discipline, and the verdict math run reliably instead of by
prose. This is critical: contaminated baselines produce false acquittals, so
the protocol can't be left to improvisation.

**Why a worktree per A/B agent:** the baseline test MUTATES shared files (it
moves every AGENTS.md/CLAUDE.md aside, runs the task on the stripped tree, then
restores them). If parallel agents ran in the same checkout they'd clobber each
other's move/restore state. The workflow gives every A/B agent
`isolation: 'worktree'` so each runs in its own checkout. Do not try to run the
baseline by hand in the live tree.

First assemble the suspect lineup from Steps 5-6 — for each rule, the realistic
task `prompt` and its 2-4 `assertions` (each `automated` grep-able or manual),
plus `control: true` on the positive-control rules. Then invoke the workflow:

```
Workflow({
  scriptPath: "${CLAUDE_PLUGIN_ROOT}/skills/roast-my-agents-md/workflow.js",
  args: {
    projectRoot: "{project-root}",
    suspects: [
      {
        id: "rule-7",
        ruleText: "<exact rule text from AGENTS.md>",
        control: false,                              // true for positive controls
        prompt: "<realistic task that would exercise the rule>",
        assertions: [
          { id: "a1", text: "<grep-able check>", automated: true },
          { id: "a2", text: "<manual judgment>", automated: false }
        ]
      }
      // ... 3-5 suspects + 1-2 controls
    ]
  }
})
```

The workflow runs three phases (all autonomous — no human gate inside it):

- **Baseline (parallel Sonnet, worktree-isolated):** each agent moves all
  instruction files aside, verifies zero remain, performs its task as a fresh
  agent with NO rules, then ALWAYS restores — and self-reports pass/fail per
  assertion with evidence.
- **WithInstruction (parallel Sonnet, worktree-isolated):** runs the **after**
  the baseline barrier, with all instruction files present and in force, and
  reports pass/fail per assertion.
- **Verdict (JS-diff, no agent):** diffs baseline vs with-instruction per
  assertion using the verdict definitions verbatim — **GUILTY** (baseline
  already passed → dead weight), **ACQUITTED** (baseline failed but
  with-instruction passed → actually useful), **MISTRIAL** (inconclusive) — and
  rolls them up per rule.

> User-level files (`~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`,
> `~/.cursor/AGENTS.md`, `~/.claude/projects/*/memory`) live OUTSIDE the project
> and outside the worktrees. They will still contaminate baselines. **ASK the
> user first** — "Found global configs that will contaminate baselines. OK to
> temporarily rename?" — and only if they consent, rename them aside before the
> Workflow call and restore them after.

It returns `{ projectRoot, tally, verdicts }` where `tally` is
`{ rulesTested, guilty, acquitted, mistrial }` and each entry in `verdicts`
carries `{ id, ruleText, control, verdict, assertions[] }` with per-assertion
`baselinePassed` / `withInstructionPassed`.

### Step 8: The Trial Results

Render the workflow's `verdicts` as evidence cards. For each suspect rule:
