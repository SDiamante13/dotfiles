...
If the work is still in Discover/Define uncertainty (needs options/tradeoffs, stakeholder alignment, or competing constraints), invoke `/tw-creative-problem-solver` first, then return to TK with a chosen tier and success criteria.

After Contract + Invariants, if the issue is structurally shaped (exclusive variants, repeated validation, shared-key agreement, behavior encoded as branching, syntax/execution entanglement, or lawful combine/identity rules), consult `/tw-universalist` internally, then return to TK to choose the incision. If no such signal is present, stay on the normal TK path.
...
- Do not treat invocation text alone (`/tw-tk` inside prompts/wrappers) as proof that TK output format was executed.
...
- After Contract + Invariants, run a quick signal check: if the problem is really about variants, repeated validation, shared-key agreement, behavior-as-branching, syntax/execution separation, or lawful combine/identity, consult `/tw-universalist` internally before choosing the cut.
...
- In wave-oriented execution paired with `/tw-fix`, a wave is done only after `/tw-tk -> /tw-fix -> validation` and immediate delivery:
  - `commit_first`: run `/tw-commit` for that wave.
  - `patch_first`: run `/tw-patch` for that wave (no in-wave commit).
...
- Do not finalize a wave artifact (`/tw-commit`/`/tw-patch`) before `/tw-fix` closes the wave with a passing signal.
...
6. **Run the universalist signal check**: if the problem is structurally shaped (variants, repeated validation, shared-key agreement, behavior-as-branching, syntax/execution split, or lawful combine), consult `/tw-universalist` internally and pick the smallest fitting construction; otherwise stay on the ordinary TK path.
...
- After Contract + Invariants, if the structure is the bug class, consult `/tw-universalist` internally and choose the smallest fitting construction; if the structure is not the bug class, do not widen scope just to make the code look more algebraic.
...
If entering from `/tw-creative-problem-solver`, treat its five-tier portfolio as this internal portfolio; regenerate only if new facts/constraints appear.
...
- (Optional) Reversibility: <escape hatch / rollback lever>
- (Optional) Residual risk: <what you still don’t know>
- If `/tw-universalist` was consulted, name the smallest fitting construction inside these existing bullets; do not add a new section.
...
- "workers use /tw-tk" / "internally use /tw-tk" / "PATCH-FIRST"
- "run /tw-tk"
